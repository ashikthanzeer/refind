...
public class LostItemFrame extends JFrame implements SessionContext.SessionListener {
    ...
    private JComboBox<LostItemEditDialog.CategoryWrapper> reportCategoryCombo;
    private JComboBox<LostItemEditDialog.LocationWrapper> reportLocationCombo;
    ...
    public LostItemFrame(ItemService itemService, CategoryService categoryService,
                         LocationService locationService, SessionContext sessionContext) {
        super("ReFind - Lost Item Management Portal");
    ...
    private void initWindow() {
        ...
        tabbedPane.addTab("Browse & Search Lost Items", buildBrowsePanel());
        tabbedPane.addTab("Report New Lost Item", buildReportPanel());
    ...
    private JPanel buildHeaderPanel() {
        ...
        JLabel subTitle = new JLabel("|  Lost Item Management Module");
    ...
    private JPanel buildBrowsePanel() {
        ...
        countLabel = new JLabel("Loading lost items...");
    ...
    private JPanel buildReportPanel() {
        ...
        JLabel title = new JLabel("Report a Lost Item");
        ...
        gbc.gridy = row++;
        JLabel lLbl = new JLabel("Location (Where lost)");
        ...
        JButton submitBtn = UITheme.createPrimaryButton("Submit Lost Item Report");
    ...
    private void populateReportCategories() {
        ...
        try {
            List<Category> list = categoryService.getAllCategories();
            for (Category c : list) {
                reportCategoryCombo.addItem(new LostItemEditDialog.CategoryWrapper(c));
            }
        } catch (Exception ignored) {}
    }
    ...
    private void populateReportLocations() {
        ...
        reportLocationCombo.addItem(new LostItemEditDialog.LocationWrapper(null));
        try {
            List<Location> list = locationService.getAllLocations();
            for (Location l : list) {
                reportLocationCombo.addItem(new LostItemEditDialog.LocationWrapper(l));
            }
        } catch (Exception ignored) {}
    }
    ...
    private void submitReport() {
        ...
        LostItemEditDialog.CategoryWrapper catWrap = (LostItemEditDialog.CategoryWrapper) reportCategoryCombo.getSelectedItem();
        ...
        LostItemEditDialog.LocationWrapper locWrap = (LostItemEditDialog.LocationWrapper) reportLocationCombo.getSelectedItem();
    ...
    private void loadLostItems() {
        try {
            ...
            tableModel.setItems(lostItems);
            countLabel.setText("Showing " + lostItems.size() + " lost item" + (lostItems.size() == 1 ? "" : "s"));
        } catch (Exception ex) {
    ...
    private void applyFilter() {
        ...
        try {
            ...
            countLabel.setText("Found " + list.size() + " lost item" + (list.size() == 1 ? "" : "s"));
        } catch (Exception ex) {
    ...
    private void viewSelectedDetails() {
        ...
        if (row == -1) {
            JOptionPane.showMessageDialog(this, "Please select a lost item from the table first.", "Selection Required", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        Item selected = tableModel.getItemAt(row);
        if (selected != null) {
            LostItemDetailsDialog dialog = new LostItemDetailsDialog(this, selected, itemService, categoryService, locationService, sessionContext, this::loadLostItems);
    ...
    private void editSelectedItem() {
        ...
        LostItemEditDialog dialog = new LostItemEditDialog(this, selected, itemService, categoryService, locationService, sessionContext, this::loadLostItems);
        dialog.setVisible(true);
    }
    ...
    private void deleteSelectedItem() {
        ...
        int choice = JOptionPane.showConfirmDialog(this,
                "Are you sure you want to delete lost item #" + selected.getId() + " - \"" + selected.getTitle() + "\"?",
        ...
        if (choice == JOptionPane.YES_OPTION) {
            try {
                ...
                if (success) {
                    JOptionPane.showMessageDialog(this, "Lost item #" + selected.getId() + " successfully deleted.", "Deleted", JOptionPane.INFORMATION_MESSAGE);
                    loadLostItems();
                }
            } catch (ValidationException ex) {
